const canvasDotsBg = function () {
  ...
  canvas.height = document.body.scrollHeight; // Changed to scrollHeight to cover full page
  ...
  function createDots() {
    ...
    // Clear the array before adding new dots
    dots.array = [];
  ...
  window.onscroll = function (parameter) {
    ...
    const top =
      (window.pageYOffset || document.scrollTop) - (document.clientTop || 0);
    mousePosition.y += top;
    // Adjust canvas height if needed when scrolling
